...
function App() {
  ...
  useEffect(() => {
    // 0) Nhận JWT từ OAuth redirect (Google/Facebook) nếu có
      ...
      fetch("http://localhost:5001/api/me", {
        ...
        .then((data) => {
          if (data && !data.error) {
            ...
            // Nếu token lỗi, dọn localStorage
        ...
        .finally(() => {
          // Xóa query param khỏi URL để tránh reload bị chạy lại
    ...
    if (oauth) {
      // Trường hợp oauth=0 hoặc có error -> chỉ dọn URL
