...
const JWT_SECRET = process.env.JWT_SECRET || "your-secret-key-123"; // Nên đặt trong biến môi trường
...
// =======================================================
// 0. SESSION + PASSPORT (OAUTH LOGIN)
// =======================================================
app.use(
  session({
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    // === LOGIC PHÂN QUYỀN ===
    // Ưu tiên dùng cột VaiTro trong DB, fallback theo TenDangNhap để tương thích dữ liệu cũ
...
app.get("/api/vip-packages", async (req, res) => {
  try {
    ...
    const packages = rows.map((pkg) => ({
      ...
      features: [
        // Tạm thời hardcode các tính năng hiển thị, hoặc bạn có thể thêm bảng 'tinhnang_goi' sau này
...
app.post("/api/payment/process", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (existingSub.length > 0) {
      ...
      newEndDate = new Date(
        currentEndDate.getTime() + durationDays * 24 * 60 * 60 * 1000
      );
      ...
      console.log(
        `User ${userId} đăng ký mới ${durationDays} ngày. Hết hạn: ${newEndDate.toISOString()}`
      );
    }
