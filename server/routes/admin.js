...
const {
  createPublicSongMediaUpload,
} = require("../middleware/publicSongMediaUpload");
...
const songMediaUpload = createPublicSongMediaUpload();
...
router.get("/songs", async (req, res) => {
  try {
    ...
    console.log("Debug pagination params:", { page, limit, offset, q: qRaw });

    const where = qRaw ? "WHERE b.TieuDe LIKE ? OR n.TenNgheSi LIKE ?" : "";
    const params = qRaw ? [like, like] : [];
    ...
    const countSql = qRaw
      ? `
        SELECT COUNT(DISTINCT b.BaiHatID) as total
        FROM baihat b
        LEFT JOIN baihat_nghesi bn ON b.BaiHatID = bn.BaiHatID
        LEFT JOIN nghesi n ON bn.NgheSiID = n.NgheSiID
        ${where}
    ...
    const totalSongs = Number(countResult?.[0]?.total || 0);
...
router.post(
  "/songs",
  songMediaUpload.fields([
    { name: "audioFile", maxCount: 1 },
    { name: "imageFile", maxCount: 1 },
  ]),
  async (req, res) => {
    const { title, artist } = req.body || {};
    const audioUrlFromBody = req.body?.audioUrl;
    const imageUrlFromBody = req.body?.imageUrl;

    const audioFile = req.files?.audioFile?.[0];
    const imageFile = req.files?.imageFile?.[0];

    const audioValue = audioFile ? audioFile.filename : audioUrlFromBody;
    const imageValue = imageFile ? imageFile.filename : imageUrlFromBody;

    if (!title || !audioValue) {
      return res.status(400).json({
        error: "Tên bài hát và file/đường dẫn nhạc là bắt buộc",
      });
    }
    ...
    try {
      ...
      const [songResult] = await connection.execute(
        ...
        [title, audioValue, imageValue || null]
      );
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Load current song to support partial updates
      const [currentRows] = await connection.execute(
        "SELECT DuongDanAudio as audioUrl, AnhBiaBaiHat as imageUrl FROM baihat WHERE BaiHatID = ?",
        [songId]
      );
      const current = currentRows?.[0] || {};

      // audio: upload wins, else use body, else keep current
      const audioValue = audioFile
        ? audioFile.filename
        : audioUrlFromBody !== undefined
        ? audioUrlFromBody
        : current.audioUrl;
      ...
      if (!audioValue) {
        ...
        return res
          .status(400)
          .json({ error: "File/đường dẫn nhạc là bắt buộc" });
      }

      // image: upload wins, else if explicitly provided in body -> set (or null if empty), else keep current
      let shouldUpdateImage = false;
      let imageValue = current.imageUrl;
      if (imageFile) {
        shouldUpdateImage = true;
        imageValue = imageFile.filename;
      } else if (imageUrlFromBody !== undefined) {
        shouldUpdateImage = true;
        imageValue = imageUrlFromBody ? imageUrlFromBody : null;
      }
      ...
      if (shouldUpdateImage) {
        await connection.execute(
          ...
          [title, audioValue, imageValue || null, songId]
        );
      } else {
        await connection.execute(
          "UPDATE baihat SET TieuDe = ?, DuongDanAudio = ? WHERE BaiHatID = ?",
          [title, audioValue, songId]
        );
      }
      ...
      if (artist && String(artist).trim() !== "") {
        ...
        await connection.execute(
          "DELETE FROM baihat_nghesi WHERE BaiHatID = ?",
          [songId]
        );
...
router.get("/users", async (req, res) => {
  try {
    ...
    const like = qRaw ? `%${qRaw}%` : null;
    const where = qRaw
      ? "WHERE TenDangNhap LIKE ? OR Email LIKE ? OR TenHienThi LIKE ?"
      : "";
    const params = qRaw ? [like, like, like] : [];
...
router.get("/artists", async (req, res) => {
  try {
    ...
    const qRaw = String(req.query.q || req.query.search || "").trim();
    const like = qRaw ? `%${qRaw}%` : null;
    const where = qRaw ? "WHERE TenNgheSi LIKE ? OR TieuSu LIKE ?" : "";
    const params = qRaw ? [like, like] : [];
    ...
    const [countRows] = await pool.execute(
      `SELECT COUNT(*) as total FROM nghesi ${where}`,
      params
    );
...
router.use((err, req, res, next) => {
  // Multer errors (file too large, invalid type, ...)
  if (err && err.name === "MulterError") {
    const code = String(err.code || "");
    if (code === "LIMIT_FILE_SIZE") {
      return res
        .status(413)
        .json({ error: "File quá lớn (vượt giới hạn upload)" });
    }
    return res
      .status(400)
      .json({ error: err.message || "Upload file thất bại" });
  }
  ...
  if (err) {
    ...
    const looksLikeBadRequest =
      ...
      msg.includes("Only audio files are allowed") ||
      msg.includes("Unsupported upload field");
    return res
      .status(looksLikeBadRequest ? 400 : 500)
      .json({ error: msg || "Lỗi server" });
  }
